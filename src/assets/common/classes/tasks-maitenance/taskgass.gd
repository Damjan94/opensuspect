...
func _ready():
	...
	#TaskMaitenanceManager.connect("maintenance_task_gas_update_input", self, "_on_update_input")
	
	#TaskMaitenanceManager.emit_signal("maintenance_task_gas_update_gui",
	#{"inputPressureRatio": inputRatio, "outputPressureRatio": outputRatio,
	#	"dialRatio": dial / (dialMaxValue - dialMinValue)})
